<template>
  <div class="login-page">
    ...
    <div class="login-container">
        ...
        <form v-if="!statusMessage" @submit.prevent="handleForgotPassword" class="login-form">
...
onMounted(() => {
  console.log("ForgotPassword.vue component mounted successfully.");
});
...
async function handleForgotPassword() {
  ...
  try {
    ...
    console.log('Attempting password reset request for:', form.email); // DEBUG

    // Make the request to a new API endpoint
    const response = await axios.post('/api/password/email', form); // Endpoint to be created
    console.log('Password reset request successful:', response); // DEBUG

    // Display success message from backend
    statusMessage.value = response.data.status || 'Password reset link sent!';

  } catch (error) {
    console.error('Password reset request failed:', error); // DEBUG
    loading.value = false;
    if (error.response) {
      if (error.response.status === 422) {
        ...
        // Other server errors
        errorMessage.value = error.response.data.message || 'Failed to send reset link. Please try again later.';
      }
    } else {
