        ...
        <form @submit.prevent="handleRegister" class="login-form">
            ...
            <div class="auth-links">
                <div class="register-link">
                    ...
                    <router-link to="/login">Log in</router-link> <!-- Use router-link -->
                </div>
            </div>
        </form>
...
// import { useStore } from 'vuex'; // Store might not be needed immediately for register

onMounted(() => {
  console.log("Register.vue component mounted successfully.");
});
...
async function handleRegister() {
  ...
  try {
    ...
    console.log('Attempting registration with:', form.email); // DEBUG

    // Make the registration request to a new API endpoint
    await axios.post('/api/register', form); // Endpoint to be created
    console.log('Registration request successful'); // DEBUG

    // Option 1: Redirect to login page after successful registration
    console.log('Registration successful, redirecting to login...'); // DEBUG
    router.push('/login?registered=true'); // Add query param to show success message?

    // Option 2: Automatically log in the user (requires backend to return user/token or session setup)
    // await store.dispatch('user/fetchUser'); // Fetch user if logged in
    // router.push('/home');
    ...
    if (error.response) {
      if (error.response.status === 422) {
        ...
        // Other server errors
        errorMessage.value = error.response.data.message || 'Registration failed. Please try again later.';
      }
    } else {
...
/* Remember Me (Not needed for register) */
...
/* Using register-link style for the "Already have an account?" link */
.register-link {
